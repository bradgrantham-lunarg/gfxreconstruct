...
constexpr uint32_t RemoveCompressedBlockType(uint32_t block_type)
{
    return ~kCompressedBlockTypeBit & block_type;
}
...
std::map<BlockType, std::string> BlockTypeToName =
{
    {kUnknownBlock, "UnknownBlock"},
    {kFrameMarkerBlock, "FrameMarkerBlock"},
    {kStateMarkerBlock, "StateMarkerBlock"},
    {kMetaDataBlock, "MetaDataBlock"},
    {kFunctionCallBlock, "FunctionCallBlock"},
    {kAnnotation, "Annotation"},
    {kMethodCallBlock, "MethodCallBlock"},
    {kCompressedMetaDataBlock, "CompressedMetaDataBlock"},
    {kCompressedFunctionCallBlock, "CompressedFunctionCallBlock"},
    {kCompressedMethodCallBlock, "CompressedMethodCallBlock"},
};
...
std::map<MetaDataType, std::string> MetaDataTypeToName =
{
    {kUnknownMetaDataType, "kUnknownMetaDataType"},
    {kDisplayMessageCommand, "kDisplayMessageCommand"},
    {kFillMemoryCommand, "kFillMemoryCommand"},
    {kResizeWindowCommand, "kResizeWindowCommand"},
    {kSetSwapchainImageStateCommand, "kSetSwapchainImageStateCommand"},
    {kBeginResourceInitCommand, "kBeginResourceInitCommand"},
    {kEndResourceInitCommand, "kEndResourceInitCommand"},
    {kInitBufferCommand, "kInitBufferCommand"},
    {kInitImageCommand, "kInitImageCommand"},
    {kCreateHardwareBufferCommand_deprecated, "kCreateHardwareBufferCommand_deprecated"},
    {kDestroyHardwareBufferCommand, "kDestroyHardwareBufferCommand"},
    {kSetDevicePropertiesCommand, "kSetDevicePropertiesCommand"},
    {kSetDeviceMemoryPropertiesCommand, "kSetDeviceMemoryPropertiesCommand"},
    {kResizeWindowCommand2, "kResizeWindowCommand2"},
    {kSetOpaqueAddressCommand, "kSetOpaqueAddressCommand"},
    {kSetRayTracingShaderGroupHandlesCommand, "kSetRayTracingShaderGroupHandlesCommand"},
    {kCreateHeapAllocationCommand, "kCreateHeapAllocationCommand"},
    {kInitSubresourceCommand, "kInitSubresourceCommand"},
    {kExeFileInfoCommand, "kExeFileInfoCommand"},
    {kInitDx12AccelerationStructureCommand, "kInitDx12AccelerationStructureCommand"},
    {kFillMemoryResourceValueCommand, "kFillMemoryResourceValueCommand"},
    {kDxgiAdapterInfoCommand, "kDxgiAdapterInfoCommand"},
    {kDriverInfoCommand, "kDriverInfoCommand"},
    {kReserved23, "kReserved23"},
    {kCreateHardwareBufferCommand, "kCreateHardwareBufferCommand"},
    {kReserved25, "kReserved25"},
    {kDx12RuntimeInfoCommand, "kDx12RuntimeInfoCommand"},
    {kParentToChildDependency, "kParentToChildDependency"},
    {kVulkanBuildAccelerationStructuresCommand, "kVulkanBuildAccelerationStructuresCommand"},
    {kVulkanCopyAccelerationStructuresCommand, "kVulkanCopyAccelerationStructuresCommand"},
    {kVulkanWriteAccelerationStructuresPropertiesCommand, "kVulkanWriteAccelerationStructuresPropertiesCommand"},
    {kReserved31, "kReserved31"},
    {kSetEnvironmentVariablesCommand, "kSetEnvironmentVariablesCommand"},
    {kViewRelativeLocation, "kViewRelativeLocation"},
    {kExecuteBlocksFromFile, "kExecuteBlocksFromFile"},
    {unknownAHBMaybe, "unknownAHBMaybe"},
};
...
void WriteReasonableGFXRHeader(FILE *fp, uint32_t compression_type)
{
    using namespace gfxrecon::format;
    std::vector<std::pair<uint32_t, uint32_t>> options { {1, compression_type} };
    FileHeader header {GFXR_FOURCC, GFXR_MAJOR, GFXR_MINOR, static_cast<uint32_t>(options.size())};
    fwrite(&header, sizeof(header), 1, fp);
    for(const auto& option: options) {
        fwrite(&option.first, sizeof(option.first), 1, fp);
        fwrite(&option.second, sizeof(option.second), 1, fp);
    }
}
...
int main(int argc, char **argv)
{
    ...
    for(uint32_t j = 0; j < header.num_options; j++)
    {
        uint32_t keyvalue[2];
        if(fread(keyvalue, sizeof(keyvalue[0]), 2, input_file) != 2)
        {
            printf("failed to read keyvalue %d from %s\n", j, input_filename);
            exit(1);
        }
        if(keyvalue[0] == 1)
        {
            compression_type = keyvalue[1];
        }
    }
